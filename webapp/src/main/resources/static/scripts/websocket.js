...
function connect(name) {
    try {
        ...
        if (! ('readyState' in socket)) {
            socket = new WebSocket(getBaseURL() + '/sastask/ws');
        }
        ...
        socket.onmessage = function(msg) {
            ...
            if(msg.data=='admin is connected.'){
              socket.send("/sessions");
            }
        }
        socket.onclose = function() {
            socket = {}; // Remove socket connection

            // Swap the term and login divs around. Note we don't do this until the connection has closed.
...
function getBaseURL() {
    ...
    var href = window.location.href.substring(window.location.href.indexOf("//")+2); // strip "http://" or "https://"
