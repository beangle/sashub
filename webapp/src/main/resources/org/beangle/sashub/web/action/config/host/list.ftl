...
[@b.grid items=hosts var="host"]
  ...
  [@b.row]
    ...
    [@b.col title="系统特性" width="15%"][#list host.features?sort_by('name') as feature] [#if feature.support(host.platform)]<image style="max-height:20px" src="${feature.logo}" title="${feature.name}"/>[/#if][#sep]&nbsp;[/#list][/@]
    [@b.col property="memory" title="内存" width="10%"]${host.memory}GB[/@]
    [@b.col property="cores" title="CPU核心" width="10%"]${host.cores}[/@]
    [@b.col property="cpu" title="CPU规格" width="25%"]${host.cpu}[/@]
  [/@]
[/@]
[@b.foot/]
