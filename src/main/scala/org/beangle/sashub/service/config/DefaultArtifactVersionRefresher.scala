...
class DefaultArtifactVersionRefresher(val expression: String) extends AbstractDaoTask, Logging, ArtifactVersionRefresher, Scheduled {
  ...
  override def refresh(artifacts: Iterable[Artifact]): Unit = {
    ...
    artifacts foreach { artifact =>
      ...
      if (res.isOk) {
        val xml = Document.parse(Networks.url(artifactMetadataUrl))
