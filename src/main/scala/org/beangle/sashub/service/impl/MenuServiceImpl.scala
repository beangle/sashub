...
class MenuServiceImpl extends MenuService {
  ...
  def importFrom(app: App, doc: Document): Unit = {
    parseMenu(app, None, doc)
  }

  private def parseMenu(app: App, parent: Option[Menu], doc: Node): Unit = {
    (doc \ "resources" \ "resource") foreach { r =>
